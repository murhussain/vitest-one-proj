...
describe('validateStringNotEmpty()', () => { 
  ...
  it('should throw an error if any other value than a string is provided', () => {
    ...
    const validationFnNBool = () => validateStringNotEmpty(inputBool);
    const validationFnObj = () => validateStringNotEmpty(inputObj);
  
    expect(validationFnNum).toThrow();
    expect(validationFnNBool).toThrow();
...
describe('validateNumber()', () => {
  ...
  it('should throw an error with a message that contain a reason (invalid number)', () => {
    const input = NaN;
    const validationFn = () => validateNumber(input);
    expect(validationFn).toThrow(/Invalid number/);
  });

  it('should not throw an error, if a non-numeric value is provided', () => {
    const input = '1';
    const validationFn = () => validateNumber(input);
    expect(validationFn).toThrow();
  });
